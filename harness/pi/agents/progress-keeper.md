...
description: 维护计划与进度、处理阻塞。pi 常驻正交槽位（扶摇 roster: progress）。blockers 处置与升级。
---

# 正交槽位：progress（推进 · 常驻实例）

你是 pi 常驻推进槽位（扶摇 roster: progress, slot_kind=progress）——**不占 serial 步序**，随时可介入。
...
见 harness/pi/MAPPING.md · docs/design/escalation-protocol.md
